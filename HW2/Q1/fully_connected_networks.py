...
class Linear_ReLU(object):
    ...
    def backward(dout, cache):
        ...
        dz_relu = ReLU.backward(dout, relu_cache) 
        dx, dw, db = Linear.backward(dz_relu, fc_cache)
...
def softmax_loss(x, y):
    ...
    exp_x = torch.exp(x)
    probs = exp_x / exp_x.sum(axis=1, keepdim=True)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h1, h1_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        h2, h2_cache = Linear.forward(h1, self.params['W2'], self.params['b2'])
        scores = h2
        ...
        loss, dscore = softmax_loss(h2, y)
        dx2, grads['W2'], grads['b2'] = Linear.backward(dscore, h2_cache)
        dx1, grads['W1'], grads['b1']  = Linear_ReLU.backward(dx2, h1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        cache = {}
        for i in range(1, L):
          out, cache['H' + str(i)] = Linear_ReLU.forward(out, self.params['W' + str(i)], self.params['b' + str(i)])
          if self.use_dropout:
            out, cache['D' + str(i)] = Dropout.forward(out, self.dropout_param)
        # Forward pass for the final layer
        scores, cache['H' + str(L)] = Linear.forward(out, self.params['W' + str(L)], self.params['b' + str(L)])
        ...
        dout, grads['W' + str(L)], grads['b' + str(L)] = Linear.backward(dout, cache['H' + str(L)])
        grads['W' + str(L)] += 2 * self.reg * self.params['W' + str(L)]
        loss += self.reg * torch.sum(self.params['W' + str(L)]**2)

        # Backward pass for hidden layers
        for i in range(L - 1, 0, -1):
          if self.use_dropout:
              dout = Dropout.backward(dout, cache['D' + str(i)])
          dout, grads['W' + str(i)], grads['b' + str(i)] = Linear_ReLU.backward(dout, cache['H' + str(i)])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # parameters chosen by trying to maximize accuracy
...
def adam(w, dw, config=None):
    ...
    t = config['t'] + 1  # Update iteration number
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Generate a dropout mask with values 0 and 1 based on the probability p
            mask = (torch.rand_like(x) > p).float()
            # Scale the input with the mask during training
            out = x * mask / (1 - p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
